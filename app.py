...
# ─── App Setup ─────────────────────────────────────────────────────────────
app = Flask(__name__)
app.secret_key = os.environ.get("SECRET_KEY", "nx-dev-secret-change-in-prod-" + str(uuid.uuid4()))
...
class Product(db.Model):
    ...
    photo       = db.Column(db.Text, default="")        # base64 data URL
    featured    = db.Column(db.Boolean, default=False)
    discount    = db.Column(db.Float, default=0)        # percentage 0-100
...
DEFAULT_SETTINGS = {
    ...
    "theme":    "#FFB700",
    "currency": "₦",
    "logo":     "",          # base64 data URL or external URL
}

DEFAULT_PRODUCTS = [
    dict(name="Wireless ANC Headphones", category="Electronics", price=45000,
         description="Studio-grade active noise cancelling with 30hr battery life. Rich bass, crystal highs.", image="🎧", stock=15, tags="New,Best Seller,Electronics", featured=True),
    dict(name="Smart LED Desk Lamp", category="Home", price=12500,
         description="Touch-controlled, 5 colour modes, USB-C charging port built-in. Eye-care certified.", image="💡", stock=30, tags="Home,Popular"),
    dict(name="Mechanical Keyboard", category="Electronics", price=38000,
         description="TKL layout, blue switches, RGB backlit, aluminium frame. N-key rollover.", image="⌨️", stock=8, tags="Gaming,Low Stock"),
    dict(name="Leather Laptop Bag", category="Fashion", price=22000,
         description="Genuine full-grain leather, fits up to 15.6\". Water resistant, lifetime warranty.", image="👜", stock=20, tags="Fashion,Premium", discount=10),
    dict(name="Bluetooth Speaker", category="Electronics", price=18000,
         description="360° immersive sound, IPX7 waterproof, 20hr playtime. Pair two for stereo mode.", image="🔊", stock=12, tags="Audio,Outdoor"),
    dict(name="Scented Soy Candle Set", category="Home", price=8500,
         description="Set of 3 luxury hand-poured soy candles. Lavender, Vanilla & Amber. 45hr burn each.", image="🕯️", stock=50, tags="Gifts,Home,Sale", discount=15),
]
...
def api_update_settings():
    ...
    for key, value in (request.json or {}).items():
        if key in DEFAULT_SETTINGS or key == "logo":
            s = Setting.query.filter_by(key=key).first()
            if s:
                s.value = value
            else:
...
def api_add_product():
    ...
    p = Product(
        ...
        tags=d.get("tags", ""), featured=bool(d.get("featured", False)),
...
def api_get_cart(user_id):
    ...
    return jsonify([cart_item_dict(i) for i in items if i.product])
...
def api_add_to_cart():
    ...
    if item:
        ...
        db.session.add(item)
    db.session.commit()
    return jsonify(cart_item_dict(item))
...
def api_admin_carts():
    ...
    for u in users:
        ...
        result.append({
            ...
            "items": [cart_item_dict(i) for i in items],
